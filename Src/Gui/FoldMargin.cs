...
// ***********************************************************************
// <copyright file="FoldMargin.cs" company="">
			...
			for (int y = 0; y < (DrawingPosition.Height + textArea.TextView.VisibleLineDrawingRemainder) / textArea.TextView.FontHeight + 1; ++y) {
				...
				if (rect.IntersectsWith(markerRectangle)) {
					...
					if (textArea.Document.TextEditorProperties.ShowLineNumbers) {
						g.FillRectangle(BrushRegistry.GetBrush(textArea.Enabled ? /*lineNumberPainterColor.BackgroundColor*/ BackColor : SystemColors.InactiveBorder),
						...
						g.FillRectangle(BrushRegistry.GetBrush(textArea.Enabled ? /*lineNumberPainterColor.BackgroundColor*/ BackColor : SystemColors.InactiveBorder), markerRectangle);
					}
