...
// ***********************************************************************
// <copyright file="TextArea.cs" company="">
...
using Zeroit.Framework.CodeEditor.TextEditor.Actions;
...
namespace Zeroit.Framework.CodeEditor.TextEditor
{
	...
	public class TextArea : Control
	{
		...
		CodeView       motherTextEditorControl;
		...
		public CodeView MotherTextEditorControl {
			get {
		...
		protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
		{
			...
			if (updateMargin != null) {
				updateMargin.Paint(g, updateMargin.DrawingPosition, Parent.BackColor);
//				clipRectangle.Intersect(updateMargin.DrawingPosition);
				...
				if (margin.IsVisible) {
					...
					if (clipRectangle.IntersectsWith(marginRectangle)) {
						...
						if (!marginRectangle.IsEmpty) {
							margin.Paint(g, marginRectangle, Parent.BackColor);
						}
					}
				}
			}
			...
			if (clipRectangle.IntersectsWith(textViewArea)) {
				...
				if (!textViewArea.IsEmpty) {
					textView.Paint(g, textViewArea, Parent.BackColor);
				}
			}
