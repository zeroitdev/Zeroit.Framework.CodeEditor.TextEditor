...
// ***********************************************************************
// <copyright file="TextView.cs" company="">
...
namespace Zeroit.Framework.CodeEditor.TextEditor
{
	...
	public class TextView : AbstractMargin, IDisposable
	{
		...
		public override void Paint(Graphics g, Rectangle rect, Color BackColor)
		{
			...
			for (int y = 0; y < (DrawingPosition.Height + VisibleLineDrawingRemainder) / fontHeight + 1; ++y) {
				...
				if (rect.IntersectsWith(lineRectangle)) {
					...
					PaintDocumentLine(g, currentLine, lineRectangle, BackColor);
				}
			}
		...
		void PaintDocumentLine(Graphics g, int lineNumber, Rectangle lineRectangle, Color BackColor)
		{
			Debug.Assert(lineNumber >= 0);
		    Brush bgColorBrush = new SolidBrush(BackColor);/* GetBgColorBrush(lineNumber);*/
			Brush backgroundBrush = textArea.Enabled ? bgColorBrush : new SolidBrush(BackColor) /*SystemBrushes.InactiveBorder*/;
